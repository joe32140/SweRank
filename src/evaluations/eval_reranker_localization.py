...
import sys
sys.path.append("/mnt/chatbot30TB/jdoo/research/cornstack/src")
...
def file_localization_results(agentless_path = None,
                              crag_bench_path_results = None,
                                reranker_results = None,
                                    ds_name = "swe-bench-lite", split = 'test', k = 5):
    ...
    elif reranker_results and reranker_results != {}:
        localized_files = reranker_results
    else:
...
def main(level = 'file', mode = 'agentless', model = 'CodeRankEmbed', ds_name = 'swe-bench-lite', split = 'test', k = 10, agentless_path = '', retriever_output_dir = 'results', reranker_output_dir = 'reranker_results', data_type="code_datasets", dataset_output_dir = 'datasets'):
    ...
    if not ((mode.startswith('agentless') and level == 'file')) :
        ...
        if (mode.startswith('agentless') and level == 'function') or not actually_file:
            ...
            prefx = f'{retriever_output_dir}/model={model}_dataset={ds_name}_split={split}_level={level}_eval_mode=default'
            crag_bench_path_results = Path(f'{prefx}_results.json')
            crag_bench_path_results = extract_file_path(convert_solutions_dict(load_json(crag_bench_path_results), key = 'docs'))
            level = 'file'
    ...
    if level == 'file':
        ...
        if mode == 'reranker':
            for ins_dir in instance_list:
                retrieved_results = load_json(os.path.join(reranker_output_dir, ins_dir, "rerank_100_llm_gen_num.json"))[0]
                get_sorted_documents_func(retrieved_results, reranker_results)
            reranker_results = extract_file_path(reranker_results)


        file_localization_results(agentless_path, crag_bench_path_results, reranker_results,
